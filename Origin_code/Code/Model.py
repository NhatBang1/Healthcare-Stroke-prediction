...
def build_calories_model(df_cal):
    ...
    test_predictions = np.clip(model.predict(X_test), 0.0, None)
...
def preprocess_stroke_data(df_str, calories_model, artifacts=None, require_target=True):
    ...
    # Calories are a non-negative physical quantity. The regressor can
    # extrapolate below zero for sparse age/BMI combinations, so constrain the
    # generated feature before it reaches either stroke model.
    df_str["Estimated_calories"] = np.clip(
        calories_model.predict(energy_features), 0.0, None
    )
